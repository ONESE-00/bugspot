(function ($) {
	...
	$(document).ready(function () {
		$('div[id^="sp-wp-tabs-wrapper_"]').each(function () {
            ...
            // Iframe wrapper 
			$('#' + tabsID + ' iframe:not(.wp-tab-iframe,[src*="omny.fm/"])').each(function (i) {
				let max_width = $(this).attr('width') > 100 ? 'max-width:' + $(this).attr('width') + 'px' : '';
				$(this).addClass('wp-tab-iframe').wrap("<div class='wp-tab-iframe-container " + tabsID + "_" + i + " ' style='" + max_width + "'></div>");
			})
			// Tab #anchoring
			$(function () {
